...
if (!isset($_SESSION['superAdminId'])) {
    die('Unauthorized access - No session ID');
}

$superAdminId = $_SESSION['superAdminId'];

// Step 2: Verify user role in the database
$sql = "SELECT role FROM tblsuperadmin WHERE superAdminId = ?";
...
if ($user) {
    var_dump($user['role']); // Check the actual value
}

// Step 3: Ensure user is a super admin (case-insensitive check)
if (!$user || strcasecmp(trim($user['role']), 'Admin') !== 0) {
    die('Unauthorized access - Not a super admin');
}

ob_start(); // Start output buffering


// Step 4: Check if student_id is provided
...
function generateStudentForm138($student_id, $conn, $outputMode = 'I') {
    ...
    $sql = "SELECT s.subjectName, g.quarter1_grade, g.quarter2_grade, 
            ...
            ORDER BY 
                ...
                s.subjectName";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $student['userId']);
    $stmt->execute();
    ...
    COUNT(DISTINCT CASE WHEN attendance = 'Present' THEN DATE(classDate) END) AS days_present,
    COUNT(DISTINCT CASE WHEN attendance = 'Absent' THEN DATE(classDate) END) AS days_absent,
    COUNT(DISTINCT DATE(classDate)) AS total_days
FROM tblattendance 
WHERE studentId = ? 
...
ORDER BY MONTH(classDate)";
    ...
    // Clear previous output to avoid errors
ob_clean();
...
$pdf->Cell(35, 6, $student['studentAge'], 'B', 0); // Dynamically display age
$pdf->Cell(25, 6, 'Sex:', 0, 0);
$pdf->Cell(45, 6, $student['studentSex'], 'B', 1); // Dynamically display sex
...
// Get studentId and sectionId from request
$student_id = $_POST['studentId'] ?? $_GET['studentId'] ?? null;
$section_id = $_POST['sectionId'] ?? $_GET['sectionId'] ?? null;

// Debugging output (uncomment if needed)
// var_dump($_SESSION['role'], $student_id, $section_id);
// exit();
...
// If Admin selects "All" students in a section
if ($student_id === 'all') {
    ...
    if (empty($students)) {
        die("No students found in the selected section.");
    }
    ...
    // Generate and merge PDFs for each student in the section
    foreach ($students as $student) {
        echo "Generating PDF for: {$student['lastName']} (ID: {$student['studentId']})<br>";

        $pdfContent = generateStudentForm138($student['studentId'], $conn, 'S');

        if (!$pdfContent) {
            echo "Error: Could not generate PDF for student ID {$student['studentId']}<br>";
            continue;
        }

        // Save to temporary file
        $tempFile = tempnam($tempDir, 'form138_') . '.pdf';
    ...
    if (empty($tempFiles)) {
        die("No valid PDFs were generated.");
    }

    // Merge all PDFs and output
    $pdf->merge('browser', 'Form138-Section' . $section_id . '.pdf');
    ...
    // Generate Form 138 for a single student in the selected section
    echo "Generating PDF for Student ID: $student_id in Section ID: $section_id<br>";
